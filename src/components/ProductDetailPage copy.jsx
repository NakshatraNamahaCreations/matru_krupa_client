...
function mapApiProduct(p) {
  return {
    ...
    name: p.name,
    shortName: p.name,
    modelNumber: p.sku || p.modelNumber || "",
    image: p.images?.[0] || p.image || sonyBraviaProduct.image,
    images: p.images?.length ? p.images : sonyBraviaProduct.images,
    rating: p.rating || 4,
    ...
    keyFeatures: p.specifications
      ...
      : p.keyFeatures || sonyBraviaProduct.keyFeatures,
...
export default function ProductDetailPage() {
  const navigate = useNavigate();
  const { id } = useParams();
  const [product, setProduct] = useState(sonyBraviaProduct);
  ...
  useEffect(() => {
    ...
    storeApi
      ...
      .then((data) => {
        ...
        if (p?._id) setProduct(mapApiProduct(p));
      })
      .catch(() => {});
  }, [id]);
    ...
    <div className="pd-page">
      ...
      <div className="breadcrumb container">
        ...
        <span
          ...
          onClick={() => navigate("/category/Televisions")}
        >
          LED Televisions
        </span>
        <span className="breadcrumb__sep">&gt;</span>

...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    // If any item lacks a real Mongo _id (e.g. static demo products), the
    // server-side cart will be empty for them. Simulate the order locally so
    // the demo flow still completes end-to-end.
    const allHaveServerId = items.every((i) => {
    ...
    if (!allHaveServerId) {
      const fakeOrder = {
        orderNumber: `MK${Date.now().toString().slice(-8)}`,
        items: items.map((i) => ({
          product: i.product,
          quantity: i.quantity,
          price: i.price,
